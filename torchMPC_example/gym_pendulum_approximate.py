...
if __name__ == "__main__":
    ...
    class PendulumDynamics(torch.nn.Module):
        def forward(self, state, perturbed_action):
            ...
            # if state is 1D, add batch dimension (1, nx)
            if state.dim() is 1 or u.dim() is 1:
                state = state.view(1, -1)
                u = u.view(1, -1)
            # if u is 2D, take first column
            if u.shape[1] > 1:
                u = u[:, 0].view(-1, 1)
    ...
    u_init = None
    render = True
    retrain_after_iter = 50 # retrain dynamics after this many iterations
    retrain_dynamics = train 
    run_iter = 500

    downward_start = True   # start from downward position
    env = gym.make(ENV_NAME).env  # bypass the default TimeLimit wrapper
    env.reset()
    if downward_start:
        env.state = [np.pi, 1]  

    # bootstrap network with random actions (train)
    # 在开始的时候先简单的训练一下网络
    if BOOT_STRAP_ITER:
